"""基于 MovieLens-1m 数据的GraphAutoEncoder"""
import numpy as np
import torch
...
from layer import Encoder, FullyConnected, Decoder
...
#encoder,fc,fc,decoder
class GraphMatrixCompletion(nn.Module):
    def __init__(self, input_dim, side_feat_dim, gcn_hidden_dim, side_hidden_dim, encode_hidden_dim, num_support=5, num_classes=5, num_basis=3):
        super(GraphMatrixCompletion, self).__init__()
        self.encoder = SumGCNEncoder(input_dim, gcn_hidden_dim, num_support, DROPOUT_RATIO)
        self.dense1 = FullyConnected(side_feat_dim, side_hidden_dim, dropout=0.,use_bias=True)
        self.dense2 = FullyConnected(gcn_hidden_dim + side_hidden_dim, encode_hidden_dim, dropout=DROPOUT_RATIO, activation=lambda x: x)
        self.decoder = Decoder(encode_hidden_dim, num_basis, num_classes, dropout=DROPOUT_RATIO, activation=lambda x: x)

    def forward(self, user_supports, item_supports, user_inputs, item_inputs, user_side_inputs, item_side_inputs, user_edge_idx, item_edge_idx):
...
model_inputs = (user2movie_adjacencies, movie2user_adjacencies, user_identity_feature, movie_identity_feature, user_side_feature, movie_side_feature, user_indices, movie_indices)
...
def expected_rmse(logits, label):
    ...
    pred_y = torch.sum(prob * SCORES, dim=1)  #将得12345分的概率乘上分数相加，得到平均得分
